...
class vizinfo:
    ...
    def setcolors(self, which, ids, rgbs):
        ...
        if not isinstance(ids, list) and not isinstance(ids, tuple):
            ids = [ids]
        if not isinstance(rgbs, list) and not isinstance(rgbs, tuple):
            rgbs = [rgbs]
        ...
        for i in range(ntypes):
            ...
            if rgbs[0] == "loop":
                ...
                red, green, blue = colors[list_colors[i % len(colors)]]
            elif ntypes == nrgbs:
    ...
    def setfills(self, which, ids, fills):
        ...
        if not isinstance(ids, list) and not isinstance(ids, tuple):
            ids = [ids]
        if not isinstance(fills, list) and \
                not isinstance(fills, tuple):
            fills = [fills]
